...
case "$1" in
  start)
    d=$(mktemp -d); cp "$2" "$d/";
    nohup "$BIN" interactive "$d" --port="$3" >/tmp/r11srv_$3.log 2>&1 &
    echo $! > /tmp/r11srv_$3.pid; sleep 5; echo "up on $3 pid $(cat /tmp/r11srv_$3.pid)";;
  stop) kill $(cat /tmp/r11srv_$2.pid) 2>/dev/null; rm -f /tmp/r11srv_$2.pid; echo stopped;;
esac
